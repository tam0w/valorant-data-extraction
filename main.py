...
def main():
    try:
            ...
            for name, info in cache_info.items():
                ...
                if info.get("exists", False):
                    if "error" in info:
                        ...
                        print(
                            f"  Status: {'Fresh' if info.get('fresh', False) else 'Stale'}"
                        )
        ...
        if args.refresh_cache:
            if args.refresh_cache == "all":
                ...
                if clear_cache(config, args.refresh_cache):
                    print(
                        f"{args.refresh_cache.title()} cache cleared. Will fetch fresh data from API."
                    )
                else:
        ...
        initialize_ocr(
            ocr_config.get("lang", ["en"]), ocr_config.get("download_enabled", True)
        )
        ...
        logger.user_output("✓ Data exported successfully!")
        ...
        logger.user_output(
            f"An error occurred. Error logs saved with ID: {logger.error_id}"
        )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Practistics - VALORANT Scrim OCR Tool"
    )

    # Input mode
    input_group = parser.add_mutually_exclusive_group()
    input_group.add_argument(
        "--read", metavar="FOLDER", help="Read screenshots from specified folder"
    )
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="Suppress user output, show only logs"
    )
